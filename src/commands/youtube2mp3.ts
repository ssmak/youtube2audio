...
const command: GluegunCommand = {
  ...
  run: async toolbox => {
    ...
    const appConfig: any = JSON.parse(fs.readFileSync(path.resolve(process.cwd(), 'package.json')).toString())
    ...
    if (typeof url === 'string' && /^https:\/\/www\.youtube\.com\/watch\?v=[a-z0-9_]+/i.test(url)) {
      const urlInfo: RegExpMatchArray = url.match(/^https:\/\/www\.youtube\.com\/watch\?v=([a-z0-9_]+)$/i)
      ...
      stream.on('info', (videoInfo: ytdl.videoInfo, videoFormat: ytdl.videoFormat) => {
        print.info(`Audio: ${videoInfo.title}`)
        print.info(`Sample rate: ${parseInt(videoFormat.audio_sample_rate) / 1000} kHz`)
      ...
      stream.on('progress', (chunk: number, downloadByte: number, totalByte: number): void => {
        const percentage = Math.floor((downloadByte / (totalByte * 1.0)) * 100)
      ...
      stream.pipe(outFileStream, { end: true })
    } else {
      // Url not pass or not a valid Youtube's url path
      print.error('Not support this type of Youtube\'s url. It should be something like this `https://www.youtube.com/watch?v=..`.')
    }
  }
}
